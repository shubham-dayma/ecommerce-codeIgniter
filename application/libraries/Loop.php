                        ...
                        <div class="item-in">
                            ...
                            <div class="right-side">
                                <a href="<?= LANG_URL . '/' . $cartItem['url'] ?>" class="item-info">
                                    <span><?= character_limiter($cartItem['title'],60) ?></span>
                                    <span class="prices">
                                        <?=
                                        $cartItem['num_added'] == 1 ? format_price($cartItem['price']) : '<span class="num-added-single">'
                                                . $cartItem['num_added'] . '</span> x <span class="price-single">'
                                                . format_price($cartItem['price']) . '</span> = <span class="sum-price-single">'
                                                . format_price($cartItem['sum_price']) . '</span>'
                                        ?>
                                    </span>
                                </a>
                            </div>
            ...
            <li class="text-center">
                <a class="go-checkout btn btn-default btn-sm" href="<?= LANG_URL . '/checkout' ?>">
                    ...
                    !empty($cartItems['array']) ? '<i class="fa fa-check"></i> '
                            . lang('checkout') . ' - <span class="finalSum">' . format_price($cartItems['finalSum'])
                ...
                foreach ($products as $article) {
                    ...
                    <div class="product-list <?= $carousel == true ? 'item' : '' ?> <?= $classes ?> <?= $active ?>">
                        <div class="inner">
                            ...
                            <div class="price">
                                <?php $price = $article['price'] != '' ? number_format($article['price'], 2) : 0 ?>
                                <span class="underline"><?= lang('price') ?>: <?= format_price($price) ?></span>
                                <?php
                            ...
                            <div class="price-discount <?= $article['old_price'] == '' ? 'invisible' : '' ?>">
                                <?= lang('old_price') ?>: <span><?= $article['old_price'] != '' ? format_price(number_format($article['old_price'], 2)) : '' ?></span>
                            </div>
